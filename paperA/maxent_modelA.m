function maxent_modelA(name,ks)
...
plotmode=1;
odir='./maxout/';
tkt = sprintf('pwregress_maxent_bumps2d_dj0_nor_Delta%d_%s_J%d_L%d_K%d_ks%d_m%d',Delta,name,J,L,Ktrain,ks,plotmode);
...
spImgs = zeros(N,N,K);
for k=1:Ktrain
    spImgs(:,:,k)=(abs(fft2(imgs(:,:,k+ks-1))).^2)/(N^2);
end
