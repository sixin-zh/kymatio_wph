...
function [filters,littlewood_final] = morlet_filter_bank_2d_per(size_in, options)
    if(nargin<2)
		options = struct;
    end
    ...
    white_list = {'Q', 'L', 'J', 'sigma_phi', 'sigma_psi', ...
		...
		'filter_format', 'littlewood','boundary','extent'};
    ...
    options = fill_struct(options, 'extent', 4);
	...
	scale = 2^((J-1) / Q - res);
	filter_spatial =  gabor_2d_period(N, M, sigma_phi*scale, 1, 0, 0, [0,0], 'double', extent);
	phi.filter = cast(real(fft2(filter_spatial)));
	phi.meta.J = J;
    littlewood_final = phi.filter.^2;
...
%             figure(p); imagesc(psi.filter{p}); colorbar; 
%             title(sprintf('Morlet real FFT \\psi: j=%d,theta=%d',j+1,theta),'FontSize',22)
   ...
   %     littlewood_final = littlewood_final + psi.filter{p}.coefft{1}.^2;
      ...
      %  for res=1:1
       %     fprintf('max of hatpsi j=%d theta=%d res=%d is %g\n', ...
        %           psi.meta.j(p), psi.meta.theta(p), res, max(psi.filter{p}.coefft{res}(:)))
%             fil_ = abs(ifft2(psi.filter{p}.coefft{res}));
%             fprintf('max of hatpsi j=%d theta=%d res=%d is %g\n', ...
%                    psi.meta.j(p), psi.meta.theta(p), res, sum(fil_(:)))
    %    end
    end
    ...
    filters.littlewood_final = littlewood_final;
end
