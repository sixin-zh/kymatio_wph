...
#                print('hatx_bc is cuda?',hatx_bc.is_cuda)
                ...
                for idx_chunk in range(self.n_chunks):
                    devid = idx_chunk % self.nGPU
                    xpsi_bc_ = xpsi_bc.to(devid)
                    # select la1, et la2, P_c = number of |la1| in this chunk
                    xpsi_bc_la1 = torch.index_select(xpsi_bc_, 1, self.idx_wph_chunks[('la1',idx_chunk,devid)]) # (1,P_c,M,N,2)
                    xpsi_bc_la2 = torch.index_select(xpsi_bc_, 1, self.idx_wph_chunks[('la2',idx_chunk,devid)]) # (1,P_c,M,N,2)
                    #print('xpsi la1 shape', xpsi_bc_la1.shape)
                    #print('xpsi la2 shape', xpsi_bc_la2.shape)
                    k1 = self.idx_wph_chunks[('k1',idx_chunk,devid)]
                    k2 = self.idx_wph_chunks[('k2',idx_chunk,devid)]
